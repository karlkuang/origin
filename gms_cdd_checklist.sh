...
check_app() {
    ...
    if [[ !(-n $normal_gms) ]]; then
        RED "check NavGo app:"
        adb shell pm list packages -f | grep com.google.android.apps.navlite
    fi
    echo
    RED "check adups ota app:"
    adb shell pm list packages -f | grep com.adups.fota
    RED "Version must be greater than or equal to 5.22"
    adb shell dumpsys package com.adups.fota | grep version
    echo
    RED "check mtkLogger app:"
    adb shell pm list packages -f | grep com.mediatek.mtklogger
    RED "Version must be greater than or equal to 4.2.0"
    adb shell dumpsys package com.mediatek.mtklogger | grep version
    GREEN "==================================="
}
...
getprop() {
 ...
 choose_gms_version() {
     ...
     gms_version_check=`adb shell getprop | grep ro.config.low_ram`

     if [[ -n "$gms_version_check" ]]; then
         result_list_for_go
         normal_gms=""
         check_app
     else
