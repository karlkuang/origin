...
import com.android.lock.mvp.contract.MainContract;
                ...
                try {
                    ...
                    if (appInfo == null || mPackageManager.getApplicationIcon(appInfo) == null) {
                        ...
                        if ((appInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0) { //判断是否是系统应用 ApplicationInfo#isSystemApp()
                            info.setSysApp(true);
                            info.setTopTitle(mContext.getResources().getString(R.string.system_app));
                        } else {
                            info.setSysApp(false);
                            info.setTopTitle(mContext.getResources().getString(R.string.third_party_app));
                        }
                    }
                    //获取推荐应用总数
                    if (info.isLocked()) {
